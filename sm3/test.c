...
int isLessThan(unsigned char* a, unsigned char* b, int num_bytes){      
...
void hash2(char* a, int b, unsigned char* out){  
  int len_a = strlen(a);
  int len_b = sizeof(b);
...
void sm3_basic_test(){
  ...
  sm3_final(h, &c);
  print_bytes(c.s, 32, "c_s");
  print_bytes(h, 32, "h");
}


void sm3_correctness_test(){
  sm3_ctx c;
  int     i,fail;
  B       h[32], m[64];    

  for (fail=0, i=0; i<10; i++) {
    memset(m,0,sizeof(m));
    m[i]=i;
    printf("m is %x\n", *m);    
...
void find_valid_nounce(long long N, const unsigned char* boundry, const char* msg){
  printf("msg len is %d\n", strlen(msg));
  B* out = malloc(32);
  ...
  while(loop < N){
    ...
    hash2(msg, loop, out);
    //printf("\r%d loop", loop);
    //fflush(stdout);
    int valid =isLessThan(out, boundry, 32);
    if (valid == 1){
        printf("found 1! %d\n", loop);
       break;
     }   
...
int main(int argc, char **argv) {
  ...
  find_valid_nounce(N, boundary, argv[1]);
  return 0;
  sm3_basic_test();
  return 0;
  sm3_correctness_test();
  return 0;
}
